...
function gap(g, m, n) {
  var primesTillRange = [], gapOfG = [];
    for(var i = m; i <= n; i++){
      if(isPrime(i)){
        primesTillRange.push(i);
      }
    }
   for(var j = 0, len = primesTillRange.length; j < len; j++){
     if(primesTillRange[j+1] - primesTillRange[j] === g){
       //alert();
       gapOfG.push(primesTillRange[j]);
       gapOfG.push(primesTillRange[j+1]);
     }
   }
   if(gapOfG.length === 0){
     return null;
   }
    // console.log([gapOfG[0], gapOfG[1]]);
    // console.log(primesTillRange);
    return [gapOfG[0], gapOfG[1]];
}
